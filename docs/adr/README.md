...
| [0012](0012-observability-export.md) | Observability export is opt-in, in-machine by default, redacted always | accepted | The OTel/Prometheus/OTLP exporter is off by default, binds `127.0.0.1` when enabled, keeps metric cardinality bounded (per-run/child provenance lives on spans), and unconditionally redacts every label through `sentinel.Redact`. |
| [0013](0013-diff-computation-library.md) | Diff computation uses hexops/gotextdiff behind a Monitor-local seam | accepted | Compute unified patches with `hexops/gotextdiff` (Apache-2.0, already in `go.sum` transitively), feed the existing `diffview.Parse`, cap inputs at 128 KiB and 4000 lines, and keep the producer Monitor-package-private for a future lift to `shared`. |
