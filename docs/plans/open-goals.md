...
| A6 | **Restore Tier-2 security monitors** — ship `examples/agents/monitors/*.md` (or change discovery) | R | **Done.** Fixed embedded roster, run-owned Start/Resume signaling, durable budget state, and offline Claude/Cursor/Codex ACP lifecycle coverage. Plan and evidence: [`a6-restore-tier2-security-monitors.md`](a6-restore-tier2-security-monitors.md). Live vendor smoke remains opt-in and was not run. The classifiers were isolated direct-SDK callers at the time this line was written; `docs/specs/26-spec-acp-only-harness/` migrated `MonitorAdapter` to ACP, closing that gap too. |
