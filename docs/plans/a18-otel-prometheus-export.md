...
(`internal/engine/engine.go:281-306`); `StepStatus.Cost` / `.Tokens` /
...
A8 dynamic foreach fan-out (per-child spend/duration histograms); A19 remote
...
is layered: OTel-standard `OTEL_*` env vars are the source of truth,
...
- Run and step **cost / tokens** are already tracked (`step.State.SpentUSD`,
  ...
  status line, `steps/<id>/result.json`, and the headless JSON envelope
...
- Fleet-scale questions — "which backend fails validation most", "how much did
  our Claude spend today across every workflow", "which step type parks on
...
adding a second run database, and without giving the exporter any influence
...
3. Publish **lifecycle events** — gate result, review requested/submitted,
   ...
   reset — as counters and (where useful) as OTel span events.
...
8. Never emit step content, prompts, transcript text, tool inputs, tool
   outputs, secret values, or command environment as label or attribute values.
...
- **Reintroducing `JIG_HARNESS` / any env-based backend selection**. Backend /
...
- **Auth beyond OTel-standard headers.** `OTEL_EXPORTER_OTLP_HEADERS` and
  `OTEL_EXPORTER_OTLP_PROTOCOL` are the whole auth story for the first
...
A18 introduces three configuration layers, evaluated in order of precedence
(rightmost wins):
...
| `JIG_TELEMETRY_PROMETHEUS_PATH` | HTTP path for `/metrics` (default `/metrics`). |
...
`Load()` (missing/corrupt → default), `Save()` (empty root is a no-op, atomic
...
`Load()` never errors — a bad file falls back to defaults so a broken
...
configuration is present, but only measurements that do not require a run
...
*engine.Manager)` calls `Subscribe()` and drains both channels in a dedicated
...
configurable via `[telemetry].metric_prefix`.
...
|---|---|---|---|
| `jig.run.started` | counter | `1` | Increments on `RunStarted`. Labels: `workflow`. |
...
| `jig.step.attempts` | histogram | `1` | `StepStatus.Attempt + 1` on terminal transitions. |
...
| `jig.foreach.expanded` | counter | `1` | Increments on `FanOutExpanded`. Additional label: `family` (declared step ID; not child IDs). |
...
preserves the same distinction — `internal/step/step.go:87-97`).
...
- `jig.step` — one child span per step attempt/iteration/generation tuple, from
  ...
  `transport`, `model`, `attempt`, `iteration`, `generation`, `outcome`,
...
Trace context is not propagated **into** agents in the first delivery (see
...
- `internal/runner` gets **zero** new imports; `metricMux` wraps `runner.Mux`
  in `cmd/jig/wire.go` and lives entirely inside `internal/telemetry`.
...
3. Implement a `NoopExporter` used when `mode == off`; provider returns
...
### Phase 3 — event-to-measurement adapter (metrics only)
...
3. Implement `jig.exporter.dropped` self-observability: when the subscriber
   cannot keep up with the drop-on-full live bus, increment this counter
...
1. Wrap the `engine.Reporter` the scheduler constructs with
   ...
   `Reporter.ToolCall` into a `jig.tool.invoked` counter increment and (when
...
1. `cmd/jig`: initialize provider before `newManager`; attach exporter after
   ...
   and headless paths.
2. `internal/tui`: read a boolean from telemetry provider and render a
...
3. `docs/observability.md` (new): full env/config table, metric catalog, span
   ...
   security stance summary, links to ADR 0012 and the T6 transcript-redaction
   discussion in `open-goals.md`.
4. ADR 0012 (new): "OTel/Prometheus export is opt-in, in-machine by default,
   redacted always."
5. Update `docs/plans/open-goals.md` to link the plan and mark A18 as planned
   (not done) with a pointer to this document.
...
and the ADR is linked from `docs/adr/README.md` (if present) and from A18 in
`open-goals.md`.
...
| 9 | Parse OTel-standard `OTEL_*` env + jig supplement (`JIG_TELEMETRY_*`) into `telemetry.Config`; env kill switch (`OTEL_SDK_DISABLED`) wins | `internal/telemetry/config.go` | 30 min |
...
| 19 | Test OTLP export against an in-process mock collector; verify protocol selection, headers pass-through, and timeout behavior | `internal/telemetry/otlp_test.go` | 30 min |
...
| 25 | Add ADR 0012 sanctioning off-machine egress with redaction and cardinality rules | `docs/adr/0012-observability-export.md` | 30 min |
...
| 29 | Update `docs/plans/open-goals.md`: A18 → planned + link to this plan; add to F. Related docs; update D. Ranked top 20 if applicable | `docs/plans/open-goals.md` | 15 min |
...
| Spans | Per-step spans are children of the per-run span; per-tool spans are children of per-step spans; span attributes include only allow-listed keys |
...
JIG_TELEMETRY_MODE=prom JIG_TELEMETRY_PROMETHEUS_ADDR=127.0.0.1:0 \
...
- **Prometheus listener defaults to `127.0.0.1`.** A blank `JIG_TELEMETRY_PROMETHEUS_ADDR`
...
- **Redaction is unconditional.** Any string label derived from user content
  ...
  deliberately spilled secret in a tool name is rendered as `[REDACTED]` on
  the wire.
...
- **Provider errors are logged once, not fatal.** OTLP dial failure, Prometheus
  bind failure, or exporter shutdown timeout logs a single warning to stderr
...
- **ADR 0012** records the stance so future changes cannot silently expand the
  export surface.
...
| Existing tests break because scheduler behavior changes under an active exporter | `EventExporter.Attach` is a read-only subscriber and does not mutate engine state; comparison test runs the same workflow with and without an exporter and asserts identical journal bytes |
...
1. `internal/telemetry` initializes cleanly for `off`, `prom`, `otlp`, and
...
4. A slow, dead, or backpressured collector never delays `RunFinished` or
   changes `jig run` exit codes; the drop counter reports observed loss.
5. `.jig/telemetry.json`, `[telemetry]`, and `OTEL_*` / `JIG_TELEMETRY_*` env
   vars are documented, validated, and precedence-ordered.
6. `docs/observability.md`, ADR 0012, and updates to `docs/engine-design.md`
...
8. `go test ./...`, `go test -race ./internal/engine ./internal/telemetry`,
   `go vet ./...`, and `go build ./cmd/jig` all pass on the change branch.
...
- [ ] Per-step spans are children of the per-run span; per-tool spans are
...
- [ ] `docs/observability.md`, ADR 0012, engine/architecture cross-links, and
