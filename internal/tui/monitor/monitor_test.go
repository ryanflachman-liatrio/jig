...
func TestMonitorStreamingPreservesBlockNavigationPosition(t *testing.T) {
	...
	if m.chatVP.YOffset() != offset || m.chatItemCursor != cursor {
		t.Fatalf("streaming moved manual block position: offset=%d cursor=%d, want %d/%d",
			m.chatVP.YOffset(), m.chatItemCursor, offset, cursor)
	}
	if m.chatAutoScroll {
...
func TestMonitorViewFitsWindow(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, want := range []string{"Human actions", "running"} {
...
func TestMonitorResizeRefits(t *testing.T) {
	...
	for _, size := range []struct{ w, h int }{{100, 30}, {70, 20}, {120, 40}} {
		...
		plain := ansiStrip(view)
		for _, want := range []string{"Human actions", "running", "Security findings"} {
			if !strings.Contains(plain, want) {
