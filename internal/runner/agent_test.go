...
func TestExecute_AcpProfile(t *testing.T) {
	...
	t.Run("MCPServers + acp caps → CapInProcessMCP rejection", func(t *testing.T) {
		...
		if !strings.Contains(res.Err, "CapInProcessMCP") {
			t.Errorf("Err = %q, want it to name CapInProcessMCP", res.Err)
		}
	})

	t.Run("real NewAcpHarness() advertises CapPermissionCallback and CapStructuredOutput", func(t *testing.T) {
		h := harness.NewAcpHarness()
		caps := h.Capabilities()
		for _, want := range []harness.Capability{harness.CapPermissionCallback, harness.CapStructuredOutput} {
			if !caps.Has(want) {
				t.Errorf("AcpHarness.Capabilities() missing %v", want)
			}
		}
		for _, reject := range []harness.Capability{harness.CapInProcessMCP, harness.CapSessionResume, harness.CapPartialStreaming} {
			if caps.Has(reject) {
